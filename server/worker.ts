...
app.post('/api/auth/register', async (c) => {
  try {
    ...
    await recordActivity('register', newUser.id, newUser.username || newUser.email || '').catch(() => {});
...
app.get('/api/tasks', async (c) => {
  try {
    const userId = (c as any).user?.id;
    // Tiered video pools: VIP gets vip_video_pool, free gets free_video_pool (fallback: video_pool)
    let vipVideoPool: string[] = [];
    let freeVideoPool: string[] = [];
    try { const vRaw = await db.getSetting('vip_video_pool'); if (vRaw) vipVideoPool = JSON.parse(vRaw); } catch { vipVideoPool = []; }
    try { const fRaw = await db.getSetting('free_video_pool'); if (fRaw) freeVideoPool = JSON.parse(fRaw); } catch { freeVideoPool = []; }
    if (freeVideoPool.length === 0) {
      try { const fRaw = await db.getSetting('video_pool'); if (fRaw) freeVideoPool = JSON.parse(fRaw); } catch { freeVideoPool = []; }
    }
...
app.get('/api/tasks/daily-task', async (c) => {
  try {
    ...
    return c.json({
      ...
      totalPlanTasks: videoTasks.length,
      vipVideoPool: Array.isArray(vipVideoPool) && vipVideoPool.length > 0 ? vipVideoPool : freeVideoPool,
      freeVideoPool,
    });
  } catch {
...
app.post('/api/admin/import-ad-tasks', async (c) => {
  try {
    ...
    for (const ch of enabled) {
      ...
      const reward = (["adsterra", "monetag"].includes(ch.id) ? 0.25 : 0.15);
...
app.get('/api/admin/funding-stats', async (c) => {
  try {
    ...
    let totalPaid = 0;
    try {
      const comps = await db.select('completions', { key: 'status', value: 'approved' });
...
app.post('/api/recharges', async (c) => {
  try {
    ...
    return c.json({ recharge: toCamel(recharge || { id: (recharge as any)?.id }), message: 'Deposit Submitted — Processing. Our AI verification system is reviewing your receipt automatically.' });
  } catch {
    return c.json({ error: 'Internal error' }, 500);
  }
});
